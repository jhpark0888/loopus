...
class ClassDetailController extends GetxController {
  static ClassDetailController get to => Get.find();
  ClassDetailController({required this.schoolClass});
  ...
  void classDetailLoad() async {
    getDetailSpec(schoolClass.career.id, "class").then((value) {
      if (value.isError == false) {
        List<Person> memberList = List.from(value.data["member"])
            .map((member) => Person.fromJson(member))
            .toList();

        schoolClass.member.value = memberList;
      } else {
  ...
  void classPostsLoad() {
    getDetailPosts(schoolClass.career.id, posts.isNotEmpty ? posts.last.id : 0)
        .then((value) {
      if (value.isError == false) {
        List<Post> postList =
